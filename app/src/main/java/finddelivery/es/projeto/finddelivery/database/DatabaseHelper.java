...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE comentario (idUser VARCHAR ( 20 ), idEstab VARCHAR ( 20 ), comentario VARCHAR(50), FOREIGN KEY(idUser) REFERENCES usuario(login), FOREIGN KEY(idEstab) REFERENCES estabelecimento(restaurante) ON DELETE CASCADE, PRIMARY KEY(idUser, idEstab));");
        db.execSQL("CREATE TABLE avaliacao (idUser VARCHAR ( 20 ), idEstab VARCHAR ( 20 ), avaliacao NUMERIC(1,1), FOREIGN KEY(idUser) REFERENCES usuario(login), FOREIGN KEY(idEstab) REFERENCES estabelecimento(restaurante) ON DELETE CASCADE, PRIMARY KEY(idUser, idEstab));");
    }
